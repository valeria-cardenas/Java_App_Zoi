...
public interface MensajeRepositorio extends JpaRepository<MensajeEntity, Integer> {
    java.util.Optional<MensajeEntity> findByImagenUrl(String imagenUrl);

    @EntityGraph(attributePaths = "autor")
    List<MensajeEntity> findByConversacionOrderByEnviadoEnAsc(ConversacionEntity conversacion);

    @Transactional
    void deleteByConversacionIn(List<ConversacionEntity> conversaciones);

    @Transactional
    void deleteByAutor(UsuarioEntity autor);
