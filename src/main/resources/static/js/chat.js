document.addEventListener("DOMContentLoaded", () => {
  ...
  if (composer && document.documentElement.classList.contains("viewer-vet")) {
    ...
    organizer.querySelector('[name="estado"]').value =
      document.documentElement.dataset.chatState || "PENDIENTE";
    organizer.querySelector('[name="prioridad"]').value =
      document.documentElement.dataset.chatPriority || "NORMAL";
    ...
    const updateReadMarks = (unread) => {
      const selector = document.documentElement.classList.contains(
        "viewer-owner",
      )
        ? ".message.owner"
        : ".message.vet";
      ...
      ownMessages.forEach((message, index) => {
        ...
        mark.textContent =
          unread === 0 || index < ownMessages.length - unread ? " ✓✓" : " ✓";
        mark.title = mark.textContent.includes("✓✓") ? "Leído" : "Enviado";
      });
    };
    const checkActivity = async () => {
      if (document.hidden || textarea?.value.trim() || input?.files?.length)
        return;
      try {
  ...
  if (composer && document.documentElement.classList.contains("viewer-owner")) {
    ...
    window.setInterval(async () => {
      ...
      try {
        ...
        if (response.ok && (await response.json()).mensajes !== initialCount)
          location.reload();
      } catch (_) {}
    }, 6000);
  }
