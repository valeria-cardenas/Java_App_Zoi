...
document.querySelectorAll('form').forEach((form) => {
    const hasUserFields = form.querySelector('input:not([type="hidden"]):not([type="submit"]), select, textarea');
    ...
    form.querySelectorAll('input, select, textarea').forEach((field) => {
        ...
        field.addEventListener('invalid', () => {
            field.setCustomValidity(field.validity.valueMissing ? 'Este campo es obligatorio.' : message);
        });
        field.addEventListener('input', () => field.setCustomValidity(''));
        field.addEventListener('change', () => field.setCustomValidity(''));
    });

    form.querySelectorAll('input[type="file"][data-max-bytes]').forEach((field) => {
        field.addEventListener('change', () => {
            const file = field.files[0];
            if (!file) return;
            const maxBytes = Number(field.dataset.maxBytes);
            const allowed = (field.dataset.fileTypes || '').split(',').filter(Boolean);
            if (file.size > maxBytes) {
                field.setCustomValidity(field.dataset.sizeError || 'El archivo supera el tamaño permitido.');
            } else if (allowed.length && !allowed.includes(file.type)) {
                field.setCustomValidity(field.dataset.error || 'El formato del archivo no está permitido.');
            } else {
    ...
    form.querySelectorAll('[data-match]').forEach((field) => {
        ...
        const validarCoincidencia = () => {
            field.setCustomValidity(field.value && original && field.value !== original.value
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (form.hasAttribute('data-confirm-empty')) {
            const vacios = [...form.querySelectorAll('input:not([type=file]):not([type=hidden]), textarea, select')]
                .filter(field => !field.required && !String(field.value || '').trim())
                .map(field => field.closest('label')?.childNodes[0]?.textContent.trim())
                .filter(Boolean);
            if (vacios.length && !window.confirm(`Los siguientes campos opcionales están vacíos:\n\n${vacios.join(', ')}\n\n¿Deseas guardar la mascota de todas formas?`)) {
                event.preventDefault();
            }
        }
    });
